...
extern "C" void impedance_driver_(int *tmstps, double *Period,
				  ...
				  double *rho, double *mu, 
                                  ...
                                  double *par1, double *par2, double *lrr, double *Z_trm, int *term_ID, int *Z_flag);
    ...
    sten_length(sten_length)
{
  ...
  for (int i=0; i<=N; i++)
  {
    ...
    dp1dr0h[i] = dfrdr0h[i]/M_PI;
      Qnew[i]    = 1e-12;//1e-6;//1.0e-40;//0.0;//5e-2;
    Anew[i]    = A0[i];
  }
...
//          fprintf(stdout,"\n");
      impedance_driver_(&tmstps,&Period,&fa1ND,&fa2ND,&fa3ND,&rho,&mu_pl,&rbot,&rmin,y,&Lr,&Fr2,&q,&g,&par1,&par2,&lrr,&Z0ND,&term_ID,&Z_flag);
...
void Tube :: bound_right (int qLnb, double k, double theta, double t, int WM)
{
  ...
  df = 0;
    ...
    // MJC: Instead of doing a 1D root finding, use the
    // 4x4 Jacobian, as documented in Mette's thesis
//    double Aavg;
//    double gamma = 0.5*k;
//
//
...
//    double k1 = pterms_half - 0.5*Qh[N-1];
//    double k2 = y[0]*k;
//    double k3 = Ah[N-1];
//    double k4 = pterms;
//    double k5 = Aold[N]+theta*R1h[N-1];
//    double k6 = Qold[N]+theta*R2h[N-1]+gamma*S2h[N-1];
//
//    double fvec_ST[4];
//    double chi[4];
  ...
  {
//      // MJC use Lax-Wendroff
//      Aavg = 0.5*(k3+xb[1]);
//
//
//      fvec_ST[0] = k1 + P(N,Aavg,WM)*k2 - 0.5*xb[0];
//      fvec_ST[1] = k4 + P(N,xb[3],WM)*k2 - xb[2];
//      fvec_ST[2] = k5 - theta*xb[0] - xb[3];
//      fvec_ST[3] = k6 - xb[2] - theta*(sq(xb[0])/xb[1] + Bh(N,xb[1],WM))
//            + gamma*(F(xb[0],xb[1]) + dBdx1h(N,xb[1],WM));
//
//      // Some of the non-constant Jacobian terms
//
//
//      chi[0] = dPdA(N,Aavg,WM)*k2;
//      chi[1] = dPdA(N,xb[3],WM)*k2;
//      chi[2] = -2.0*theta*(xb[0]/xb[1]) + gamma * dFdQ(xb[1]);
//      chi[3] = theta*(sq(xb[0]/xb[1]) + dBdAh(N,xb[1],WM))
//            + gamma * (dFdA(xb[0],xb[1])+d2BdAdxh(N,xb[1],WM));
//
//
...
//      int ch = zero (xb, 4, 1.0e-8, 1.0e-8, fvec_ST, fjac_ST);
//      if (ch == 1) ok = true;
...
//       xb[3] = Anew[N-1]; // Bound xr away from zero.
//     }
//     j = j+1;
  ...
  Qnew[N]    = k*pL[qLnb_1]*y[0] + pterms;
    ...
    // Solution if you use Lax-Wendroff
//    Anew[N]    = xb[3];
//    pL[qLnb_1] = P(N,Anew[N],WM); // ADD P0???
//    Qnew[N]    = k*pL[qLnb_1]*y[0] + pterms;//xb[2];//k*pL[qLnb_1]*y[0] + pterms;
...
void Tube :: call_junc (double theta, double gamma, Tube *Arteries[], int parent, int WM, double t)
{
    junction(theta,gamma,Arteries,parent,WM,t,Period);
}
...
//               fprintf(stdout,"IN BOUND junc\n");
            Arteries[i] -> call_junc (theta, gamma, Arteries, i, WM,t);
          }
        }
