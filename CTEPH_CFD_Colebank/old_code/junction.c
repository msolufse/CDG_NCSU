...
//        bound_sten(theta, gamma, Arteries, parent,D1,WM);
//       fprintf(stdout,"Call screen with D1:%d D2:%d D3:%d\n",D1,D2,D3);
//         bound_screen(theta, gamma, Arteries, parent,D1,WM);
//          bound_screen2(theta, gamma, Arteries, parent,D1,WM);
...
//    fprintf(stdout,"DISCREP:%lf\n",(xb[6]-xb[9])/xb[6]);
      ...
      if (j >=ntrial) error ("arteries.C","Root not found in the monf");
}
...
///////////////////////////////////////////////////////////////
/* ADDED BY MJC: Create a solver for fluid flow through a mesh screen. This
    is intended to simulate flow through a web-like lesion in CTEPH.
   7/21/2020
 */
...
//    FILE *test = fopen("test_junction", "w");
//    for (int i=0; i<PV->N; i++) {
//        fprintf(test, "%13.10f %15.10f %13.10f %15.10f\n", PV->Qold[i], PV->Qnew[i],PV->Aold[i], PV->Anew[i]);
//    }
//    fprintf(test, "0 0 0 0\n");
//    for (int i=0; i<B1->N; i++) {
//        fprintf(test, "%13.10f %15.10f %13.10f %15.10f\n", B1->Qold[i], B1->Qnew[i],B1->Aold[i], B1->Anew[i]);
//    }
//    fclose(test);
    ...
    // MJC: Try setting area initial conditions to be less
//    xb[9]  = xb[9]*0.99;
//    xb[10] = xb[10]*0.99;
//    xb[11] = xb[11]*0.99;
...
//                for (int k=0; k<12; k++)
//                {
//                 fprintf(stdout,"%lf ",xb[k]);
//                }
//                fprintf(stdout,"\n");
//            }
//        }
        
        // Test adding jitter term
//        for(int i=0; i<12; i++)
//            if (fjac[i][i]<1e-16) {
//                fjac[i][i]=1e-16;
//            }
...
//        fprintf(stdout,"AREA DIFFERENCE: %lf\n",xb[6]-xb[9]);
//        fprintf(stdout,"Flow DIFFERENCE: %lf\n",xb[0]-xb[3]);
        // Check whether solution is close enough. If not run the loop again.
        // int ch = zero (xb, 18, 1.0e-4, 1.0e-4, fvec, fjac);
        int ch = zero (xb, 12, 1.0e-6, 1.0e-6, fvec, fjac);
        if (ch == 1) ok = true;
//        fprintf(stdout,"AREA DIFFERENCE 2: %lf \n \n",xb[6]-xb[9]);
...
//   fprintf(stdout,"Delta A:%lf\n",(xb[9]-xb[6])/xb[6]);
//    fprintf(stdout,"Delta Q:%lf\n",(xb[3]-xb[0])/xb[0]);
      if (j >=ntrial) error ("arteries.C","Root not found in the screen");
}
...
void bound_screen2(double theta, double gamma,Tube *Arteries[], int parent, int D1, int WM)
{
    ...
    while (j <= ntrial && ok==false) // Find the zero
    {
        ...
        // Define the pressure loss terms. Order is viscous, turbulent, and inertial
            term1A = dp1 * xb[1]/sq(xb[7]);
          
            term2A = (dp2 / sq(xb[7])) * ab(xb[1])*xb[1]; // Using absolute value
          
            term3A = (dp3/xb[7]) * (xb[1]-(PV->Qold[N]))/(2.0*gamma);
          
            delta_PA =  term1A + term2A + term3A;
            
        
            term1B = dp1 * xb[0]/sq(xb[6]);
          
            term2B = (dp2 / sq(xb[6])) * ab(xb[0])*xb[0]; // Using absolute value
          
            term3B = (dp3/xb[6]) * (xb[0]-(PV->Qold[N]))/(2.0*gamma);
...
//        fprintf(stdout,"parent: %d, daughter: %d",parent,D1);
//        fprintf(stdout,"Ls:%lf, Kdarcy: %lf, Kforch: %lf rho: %lf mu: %lf\n",Ls,k_darcy,k_forch,rho,mu);
//        fprintf(stdout,"xb0:%lf, xb1: %lf, xb6: %lf xb6: %lf\n",xb[0],xb[1],xb[6],xb[7]);
...
//    if (j<1000)
//    {
//        fprintf(stdout,"Ves: %d j: %d alam: %5.12lf   new: %5.12lf   old:  %5.12lf   \n",parent,j,alam,SSE_new,SSE_old);
//        for (int i=0; i<18; i++) fprintf(stdout,"p %d: %5.12lf     ",i,pln[i]);
//        fprintf(stdout,"\n");
//    }
  }
...
//// Original bound_sten without linesearch
//void bound_sten_bif (double theta, double gamma, Tube *Arteries[], int parent, int D1, int D2, int WM)
//{
//    // MJC: Try to define the daughters here
//    Tube* PV   = Arteries[ parent];
