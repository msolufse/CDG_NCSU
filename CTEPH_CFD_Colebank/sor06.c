...
int main(int argc, char *argv[])
{
    ...
    fs3   = atof(argv[6]);
    f3_sten = f3*10.0;
    f3_LV_down = f3*2.0;//5.0;
    f3_SV_down = fs3*10.0;//*10.0;
    alpha = atof(argv[7]);
    beta  = atof(argv[8]);
    lrr   = atof(argv[9]);
    rm    = atof(argv[10]);
    Z0    = atof(argv[11]);
    ves_stiff = atoi(argv[12]);
    int cycles = 1; // NOTE: Can make this a parameter to pass in

    /////// Load in network parameters
    total_vessels  = atoi(argv[13]);
    total_terminal = atoi(argv[14]);
    total_sten     = atoi(argv[15]);
    total_screen   = atoi(argv[16]);
    num_pts        = atoi(argv[17]);
...
///// A hard code for a single vessel
    if (total_vessels == 1){
        ...
        for (int i=total_vessels-1; i>=0; i--) {
            if (i==0) {
//                fprintf(stdout,"At first vessel %d with dimensions %lf %lf %lf  and daughters %d  %d\n",i,dimensions_matrix[i][0],dimensions_matrix[i][1],dimensions_matrix[i][2], curr_d1, curr_d2);
...
//                    fprintf(stdout,"At terminal vessel %d with dimensions %lf %lf %lf\n",i,dimensions_matrix[i][0],dimensions_matrix[i][1],dimensions_matrix[i][2]);
                    if (i == vessels_path[path_id]) {
                       fprintf(stdout,"STIFFER with RMIN: %lf\n",new_rm[path_id]);
                        Arteries[i] = new Tube( dimensions_matrix[i][0], dimensions_matrix[i][1], dimensions_matrix[i][2],
                        daughter_ptr, new_rm[path_id], num_pts, 0, 0,f1,f2,f3_LV_down,fs1,fs2,f3_SV_down, new_alpha[path_id], new_beta[path_id], lrr,Z0_rem,term_id,ST_calc,0,0);
                        path_id--;
                    } else {
                        Arteries[i] = new Tube( dimensions_matrix[i][0], dimensions_matrix[i][1], dimensions_matrix[i][2],
...
//                        fprintf(stdout,"At vessel %d with dimensions %lf %lf  and daughters %d  %d\n",i,dimensions_matrix[i][0],    dimensions_matrix[i][1], curr_d1, curr_d2);
                    if (i==sten_ves[sten_id][0]){
                        fprintf(stdout,"STEN BIF: %lf %lf %lf %lf\n",sten_ves[sten_id][0],sten_ves[sten_id][1],sten_ves[sten_id][2],dimensions_matrix[i][0]-sten_ves[sten_id][2]);
                        Arteries[i] = new Tube( dimensions_matrix[i][0]-sten_ves[sten_id][2], dimensions_matrix[i][1], dimensions_matrix[i][2],
                        daughter_ptr, 0, num_pts, 0, 0, f1,f2,f3_sten,0,0,0,0,0,0,0,0,ST_calc,sten_ves[sten_id][1],sten_ves[sten_id][2]);
                        sten_id--;
                    }
                        ...
                        Arteries[i] = new Tube( dimensions_matrix[i][0], dimensions_matrix[i][1], dimensions_matrix[i][2],
                        daughter_ptr, 0, num_pts, 0, 0, f1,f2,f3_sten,0,0,0,0,0,0,0,0,ST_calc,screen_ves[screen_id][1],screen_ves[screen_id][2]);
                        screen_id--;
                    }
                    else if(i == vessels_path[path_id])
                    {
                        path_id--;
                        fprintf(stdout,"STIFFER\n");
                            Arteries[i] = new Tube( dimensions_matrix[i][0], dimensions_matrix[i][1], dimensions_matrix[i][2],
                            daughter_ptr, 0, num_pts, 0, 0,f1,f2,f3_LV_down,0,0,0,0,0,0,0,0,ST_calc,0,0);
                    }
                    else {
                    Arteries[i] = new Tube( dimensions_matrix[i][0], dimensions_matrix[i][1], dimensions_matrix[i][2],
...
//   finaltime = period_counter*Period;
    ...
    const char *name1       = "pu1.2d";
    const char *name2       = "pu2.2d";
