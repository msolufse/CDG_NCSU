...
function [sens, y, sol] = senseq(pars,x,Init)

[sol,y] = model_sol(pars,x,Init);

%Depending on form of y, the sensitivity matrix could be dymodel/dpars or 
%dr/dpars where r = ymodel - ydata

[sens] = diffjac(pars,@myfun,y,x,Init);


function y = myfun(pars,x,Init)

[~,y] = model_sol(pars,x,Init);

    
function [jac] = diffjac(x,f,f0,time,Init)
...
for j = 1:n
    zz = zeros(n,1);
    zz(j) = 1;
    jac(:,j) = dirder(x,zz,f,f0,time,Init);
end

function z = dirder(x,w,f,f0,time,Init)
