...
extern "C" void impedance_driver_(int *tmstps, double *Period,
                  ...
                  double *rho, double *mu,
                                  ...
                                  double *alpha, double *beta, double *lrr1, double *lrr2, int *term_ID);
...
Tube :: Tube (double Length,
              ...
              double f1, double f2, double f3, double fs1, double fs2, double fs3, double alpha_in, double beta_in, double lrr1_in, double lrr2_in, int terminal_id):
    L(Length),
    rtop(topradius),
    rbot(botradius),
    LD(LeftDaughter),
...
//          fprintf(stdout,"\n");
//       printf("r_root: %lf, r_min: %lf, alpha: %lf, beta: %lf, lrr: %lf\n",rbot,rmin,alpha,beta,lrr);
    impedance_driver_(&tmstps,&Period,&ffs1,&ffs2,&ffs3,&rho,&mu_pl,&rbot,&rmin,y,&Lr,&Fr2,&q,&g,&alpha,&beta,&lrr1,&lrr2,&term_ID);
...
{
    fprintf (fd, "%13.10f %13.10f %15.10f %15.10f %15.10f %17.10f\n", t*Lr3/q, i*h*Lr, (P(i,Anew[i])+p0)*rho*g*Lr/conv, Qnew[i]*q, Anew[i]*Lr2, c(i, Anew[i])*10);
}
  ...
  for (int i=0; i<N; i++)
  {
    //fprintf (fd, "%13.10f %13.10f %15.10f\n",
      ...
      fprintf (fd, "%13.10f %13.10f %15.10f %15.10f %15.10f %17.10f\n",
               ...
               t*Lr3/q, (i+offset)*h*Lr, (P(i,Anew[i])+p0)*rho*g*Lr/conv, Qnew[i]*q, Anew[i]*Lr2, c(i, Anew[i])*10);
  }
}
...
double Tube :: P (int i, double A)
{
  ...
  double pold = fr[i]*(1-sqrt(A0[i]/A)); /////ADD P0?????
