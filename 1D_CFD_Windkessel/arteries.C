...
double Tube :: dBdx1 (int i, double A)
{
  ...
  double dfr = dfrdr0[i];
    double pold =  dr0dx[i]/Fr2*(
                   sqrt(A)*(dfr*sqrt(A0[i])+fr[i]*sqrt(M_PI))-
                   sqrt(A0[i])*(dfr*sqrt(A0[i]) + 2.0*fr[i]*sqrt(M_PI)));
...
double Tube :: dBdx1h (int i, double A)
{
  ...
  double dfr = dfrdr0h[ip1];
    ...
    double pold =  dr0dxh[ip1]/Fr2*(
                   sqrt(A)*(dfr*sqrt(A0h[ip1])+frh[ip1]*sqrt(M_PI))-
                   sqrt(A0h[ip1])*(dfr*sqrt(A0h[ip1]) + 2.0*frh[ip1]*sqrt(M_PI)));
...
double Tube :: d2BdAdxh (int i, double A)
{
   ...
   double dfr = dfrdr0h[ip1];
    ...
    double pold = (0.5*(dfr*sqrt(A0h[ip1])+
                  frh[ip1]*sqrt(M_PI))/sqrt(A))*dr0dxh[ip1]/Fr2;
